var c=Object.defineProperty;var d=(o,e,r)=>e in o?c(o,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):o[e]=r;var i=(o,e,r)=>d(o,typeof e!="symbol"?e+"":e,r);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const a of t)if(a.type==="childList")for(const n of a.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function r(t){const a={};return t.integrity&&(a.integrity=t.integrity),t.referrerPolicy&&(a.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?a.credentials="include":t.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(t){if(t.ep)return;t.ep=!0;const a=r(t);fetch(t.href,a)}})();class u{constructor(e){i(this,"device");i(this,"context");i(this,"canvasFormat");i(this,"vertexBuffer");i(this,"indexBuffer");i(this,"uniformBuffer");i(this,"uniforms");i(this,"shaderModule");i(this,"bindGroupLayout");i(this,"bindGroup");i(this,"pipeline");i(this,"renderPassDescriptor");i(this,"MAX_SIZE",100);this.canvas=e}async init(){await this.getGPU(),this.connectCanvas(),this.loadShaders(),this.createBuffers(),await this.createPipeline(),this.createRenderPassDescriptor(),this.render()}async getGPU(){if(!navigator.gpu){this.onError("WebGPU not supported on this browser.");return}const e=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(!e){this.onError("No GPU Adapter found.");return}this.device=await e.requestDevice()}connectCanvas(){const e=this.canvas.getContext("webgpu");if(!e){this.onError("Failed to get canvas context :/");return}this.context=e,this.canvasFormat=navigator.gpu.getPreferredCanvasFormat(),this.context.configure({device:this.device,format:this.canvasFormat})}loadShaders(){this.shaderModule=this.device.createShaderModule({label:"Ray tracing shader",code:`
                ...
                `})}createBuffers(){const e=new Float32Array([-1,-1,1,-1,-1,1,1,1]);this.vertexBuffer=this.device.createBuffer({label:"Vertex Buffer",size:e.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});const r=new Uint16Array([0,1,2,1,3,2]);this.indexBuffer=this.device.createBuffer({label:"Index Buffer",size:r.byteLength,usage:GPUBufferUsage.INDEX|GPUBufferUsage.COPY_DST}),this.uniforms=new Float32Array(this.MAX_SIZE),this.uniformBuffer=this.device.createBuffer({label:"Uniform buffer",size:this.uniforms.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.device.queue.writeBuffer(this.vertexBuffer,0,e),this.device.queue.writeBuffer(this.indexBuffer,0,r)}async createPipeline(){const e={arrayStride:8,attributes:[{format:"float32x2",offset:0,shaderLocation:0}]};this.bindGroupLayout=this.device.createBindGroupLayout({label:"Raytracer bind group layout",entries:[{binding:0,visibility:GPUShaderStage.VERTEX,buffer:{}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"filtering"}},{binding:2,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d",multisampled:!1}}]});const r=this.device.createSampler(),t=await(await fetch("./flowers.jpg")).blob(),a=await createImageBitmap(t),n=this.device.createTexture({label:"floor image",size:[a.width,a.height,1],format:"rgba8unorm",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.RENDER_ATTACHMENT});this.device.queue.copyExternalImageToTexture({source:a},{texture:n},[a.width,a.height]);const l=n.createView();this.bindGroup=this.device.createBindGroup({label:"Vertex Bind group",layout:this.bindGroupLayout,entries:[{binding:0,resource:{buffer:this.uniformBuffer}},{binding:1,resource:r},{binding:2,resource:l}]});const f=this.device.createPipelineLayout({label:"Raytracer Pipeline Layout",bindGroupLayouts:[this.bindGroupLayout]});this.pipeline=this.device.createRenderPipeline({label:"Raytracing pipeline",layout:f,vertex:{module:this.shaderModule,entryPoint:"vertexMain",buffers:[e]},fragment:{module:this.shaderModule,entryPoint:"fragmentMain",targets:[{format:this.canvasFormat}]},primitive:{topology:"triangle-list"}})}createRenderPassDescriptor(){this.renderPassDescriptor={label:"Render Pass Description",colorAttachments:[{view:this.context.getCurrentTexture().createView(),clearValue:[.2,.2,.2,1],loadOp:"clear",storeOp:"store"}]}}render(){this.renderPassDescriptor.colorAttachments[0].view=this.context.getCurrentTexture().createView();const e=this.device.createCommandEncoder(),r=e.beginRenderPass(this.renderPassDescriptor);r.setIndexBuffer(this.indexBuffer,"uint16"),r.setPipeline(this.pipeline),r.setVertexBuffer(0,this.vertexBuffer),r.setBindGroup(0,this.bindGroup),r.drawIndexed(6),r.end(),this.device.queue.submit([e.finish()]),requestAnimationFrame(()=>this.render())}onError(e){document.body.innerHTML=`<p>${e}</p>`,console.error(e)}}const v=document.getElementById("raytracer_canvas"),h=new u(v);h.init();
